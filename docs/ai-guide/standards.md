...
- **需求拆解**：功能/非功能/约束/风险（安全、隐私、可维护性、兼容性）
- **可验证指标**：对应的 tests / perf / race / correctness 目标
- **验证计划**：怎么跑、跑什么、通过标准是什么（尽量自动化）

### Implementation（实现）

- **模型负责**：生成可审阅的 patch（或差异说明）、提交信息草案、测试脚本草案。
- **同学负责**：代码审阅、风险分析、取舍决策、最终合入（你对最终结果负责）。

建议约束模型输出为“可审阅形态”，例如只允许输出 diff/patch，而不是“贴一坨最终文件”。

### Proof（验证）

你的结果必须能被复现与检查，至少包含：

- **自动化测试**：单测/集成测试/脚本化回归（能一键运行更佳）
- **最小复现**：对 bug 修复/性能问题，给出最小复现用例或步骤
- **观测证据**：trace/log/benchmark 结果（建议记录命令、环境与关键输出）
