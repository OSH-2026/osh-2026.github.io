...
## 内核态 Echo Server（10% + 5%）

本选做要求实现一个运行在 Linux 内核态的 TCP Echo Server。模块加载后，应在内核态创建监听 socket，接受用户态客户端连接，接收客户端发送的字符串，并将相同内容原样发送回客户端。该选做不要求与前面的 HTTP Server 共用代码，可以作为 `lab3/kecho` 下的独立内核模块项目提交。

本项分为两部分：

- 完成基础 Echo Server 功能，额外加 10%；
- 在基础 Echo Server 上增加可观测的统计信息，额外加 5%。

我们提供了一个不完整的参考骨架 [`kecho.reduced.c`](kecho.reduced.c)，你可以基于它补全实现，也可以自行组织代码。

### 基础 Echo Server（10%）

你需要完成以下功能：

1. 编写并编译一个 Linux 内核模块，例如生成 `kecho.ko`；
2. 模块加载后在内核态创建 TCP 监听 socket，建议监听 `0.0.0.0:8888`；
3. 使用内核线程或其他合理机制接受客户端连接；
4. 收到客户端消息后，将消息原样发回；
5. 模块卸载时能停止后台执行流并释放 socket 等资源；
6. 提供一个简单的用户态客户端或清晰的测试方法，用于证明 echo 功能正确。
...
如果客户端输出的接收内容与发送内容一致，即说明基础 echo 功能正常。你也可以使用 `nc 127.0.0.1 8888` 进行手工测试。

### 统计信息（5%）

在完成基础 Echo Server 的基础上，你可以继续实现统计信息。统计信息需要能够从用户态观察，推荐通过 `/proc/kecho` 暴露，例如：
...
如果统计数据会被内核线程、连接处理逻辑和 procfs 读取逻辑共同访问，请使用互斥锁等同步机制保护共享状态。

### 提交与报告要求

若选择该选做，请将相关代码放在 `lab3/kecho` 目录，并在报告中说明：

1. 依赖环境，例如内核头文件安装方式；
2. 编译方法；
3. 模块加载和卸载方法；
4. 客户端或 `nc` 测试方法；
5. 若实现统计信息，展示 `cat /proc/kecho` 的输出并解释各统计项。

注意：

- 加载和卸载内核模块需要 `sudo` 权限；
- 内核模块代码出错可能影响系统稳定性，建议在虚拟机中完成和测试；
- 请确保模块卸载后 `/proc/kecho` 被删除，后台线程和 socket 被正确释放；
- 统计信息部分必须建立在可正常工作的 echo 功能之上。
