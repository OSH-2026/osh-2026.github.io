...
本节我们将为 Linux 添加一个自定义的 syscall，来获得一个字符串 `"Hello, world from PB24xxxxxx's new syscall!\n"`（注意这里要把 `PB24xxxxxx` 改成你的学号）。

首先回到 linux 源码文件夹 linux-6.19.10，添加一个文件 `custom/hello.c`：
...
{
    static const char s[] = "Hello, world from PB24xxxxxx's new syscall!\n";
    // 注意这里要把 PB24xxxxxx 改成你的学号
    if (strlen(s) <= buf_len) {
...
接下来我们将此文件夹添加到 Linux 的编译工具链中。修改 `linux-6.19.10/Makefile` 文件，找到
...
- 长度充足时，返回 0，buffer 中存放完整的字符串 `"Hello, world from PB24xxxxxx's new syscall!\n"`（注意这里要把 `PB24xxxxxx` 改成你的学号）；
- 长度不充足时，返回 -1；
