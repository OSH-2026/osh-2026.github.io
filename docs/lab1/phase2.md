...
创建一个新的目录（和 `linux-6.19.10` 文件夹平级，比如名为 `temp`）用于暂存文件，再在此临时目录下创建一个 C 程序 `init.c`，代码如下：

```c
#include <stdio.h>

int main() {
    printf("Hello! PB24XXXXXX\n"); // Your Student ID
    while (1) {}
}
```
...
编译，**静态链接** 为可执行程序（这是因为我们的环境中没有加入动态链接库）：
...
```bash
qemu-system-x86_64 -kernel linux-6.19.10/arch/x86_64/boot/bzImage -initrd initrd.cpio.gz
```
