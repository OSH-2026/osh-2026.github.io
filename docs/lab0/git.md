...
**如果你需要使用 AI 工具，请确保 AI 工具不会意外地在你的 Git 仓库上执行危险操作。任何关于代码库的修改都必须先经过确认。** 更多关于此的信息请参考 AI 使用说明。
...
在创建或修改 `.gitignore` 文件后，请运行以下命令，将 已经被 Git 追踪（已加入索引）的文件 从 Git 的索引中移除（不会删除本地文件）。随后根据 `.gitignore` 中的新规则，重新添加需要被追踪的文件，并排除不需要的文件：
...
- 假定你有若干电脑，只要把每台电脑的 Key 都添加到 GitHub，就可以在每台电脑上往 GitHub 推送了。**不要在不同的电脑间共享私钥**！
...
- **「必做」**在你自己的账号下（而不是 OSH-2026 组织）创建一个新的、**私有的** GitHub 远程仓库，将其命名为 osh-2026-labs，然后邀请助教（id 分别为：ustcljh、zucanlv）成为这个仓库的共同贡献者（collaborators），以便后续实验的批改。
