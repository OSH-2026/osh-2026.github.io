...
- 以一致且良好的编程风格编写程序，可在符合相关要求的前提下使用 AI 进行辅助；
- 为源代码添加清晰、规范、风格一致的注释，读懂他人为源代码添加的注释；
- 学会正确理解和处理编译器提供的各类提示信息（建议在编写程序时启用最高警告级别，并开启运行时检查，以避免潜在问题）；
- 能够及时、准确地定位程序编写与调试过程中出现的错误和异常；
- 掌握并实践团队协作中的源代码管理方法（参见 Git 相关内容）。
...
???+ tips "选择 Rust"
    ...
    在操作系统课程或系统软件项目中讨论用 Rust 时，需要正确理解**语言安全特性**与**系统设计安全**之间的关系。Rust 通过所有权、借用检查和生命周期机制，在语言层面保证内存安全，例如能够在编译期避免内存泄漏、悬垂指针等问题，这些特性可以减少像 C 或者旧式 C++ 在工程实践中因使用不当而产生的内存错误，因此成为许多系统项目选择 Rust 的重要动力。然而，语言提供的安全保证只作用于实现层面，并不能替代系统架构层面的安全设计。在操作系统中，诸如内核与用户空间隔离、权限边界和资源访问控制等机制，才是保证系统完整性的基本原则。如果仅仅依赖 Rust 的语言安全，而在系统结构上忽视这些基本要求，那么即使代码本身是“安全 Rust”，整体系统依然可能是不安全的。因此应强调：**Rust 的价值在于减少实现错误，但它不能替代正确的操作系统安全模型；语言安全与系统设计安全必须同时得到保障**。
...
- （这不是作业的一部分）使用 C++ 编写一个 `split` 字符串函数，函数签名如下：

  ```C++
  std::vector<std::string> split(std::string s, const std::string& d);
  ```

  该函数在 s 中查找 d（单纯文本匹配），并用 d 将 s 分割成一系列子字符串。如调用 `split("1,2,3", ",")` 后，会返回一个包含 string 对象 `"1", "2", "3"` 的 vector。
